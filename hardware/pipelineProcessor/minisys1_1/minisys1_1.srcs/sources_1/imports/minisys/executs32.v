  ...
  //  assign Zero = (ALU_output_mux[31:0]== 32'h00000000) ? 1'b1 : 1'b0;
    ...
    always @(ALU_ctl or Ainput or Binput) begin
        case(ALU_ctl)
            ...
            3'b010:ALU_output_mux =  Ainput + Binput;//ADD,ADDI，LW,SW//lb,lbu,lh,lhu,sb,sh
    ...
    //溢出
    assign overflow=(~ALU_ctl[2]&&ALU_ctl[1]&&~ALU_ctl[0]&&~Ainput[31]&&~Binput[31]&&ALU_output_mux[31])
                 ||(~ALU_ctl[2]&&ALU_ctl[1]&&~ALU_ctl[0]&&Ainput[31]&&Binput[31]&&~ALU_output_mux[31])
                 ||(ALU_ctl[2]&&ALU_ctl[1]&&~ALU_ctl[0]&&~Ainput[31]&&Binput[31]&&ALU_output_mux[31])
                 ||(ALU_ctl[2]&&ALU_ctl[1]&&~ALU_ctl[0]&&Ainput[31]&&~Binput[31]&&~ALU_output_mux[31]);
    ...
    always @(negedge clock or posedge reset) begin
        ...
        end else if(((ALU_ctl==3'b111) && (Exe_code[3]==1))||((ALU_ctl[2:1]==2'b11) && (I_format==1))) //slti(sub)  处理所有SLT类的问题
    ...
    assign mult_op=Function_opcode[1:0];
    ...
    always @(*) begin
        ...
        2'b10:begin
                ALU_HI=$signed(Ainput) % $signed(Binput);
                ALU_LO=$signed(Ainput) / $signed(Binput);
            end
        2'b11:begin
                ALU_HI=Ainput % Binput;
                ALU_LO=Ainput / Binput;
            end
        endcase
    end
    ...
    always @(negedge clock or posedge reset) begin
        if(reset) begin
            hi_data=32'hxxxxxxxx;
            lo_data=32'hxxxxxxxx;
        end else if(Function_opcode[5:3]==3'b011)begin
            hi_data=ALU_HI;
            lo_data=ALU_LO;
        end else if(Function_opcode==6'b010001) begin
            hi_data=Read_data_1;
        end else if(Function_opcode==6'b010011) begin
            lo_data=Read_data_1;
        end     
    end
...
//    end
    ...
    assign MemEnable0=(((lb||lbu||sb)&&(ALU_Result[1]==0)&&(ALU_Result[0]==0))||
                        ((lh||lhu||sh)&&(ALU_Result[1]==0)&&(ALU_Result[0]==0))||
                        ((lw||sw)&&(ALU_Result[1]==0)&&(ALU_Result[0]==0)));
                        
    assign MemEnable1=(((lb||lbu||sb)&&(ALU_Result[1]==0)&&(ALU_Result[0]==1))||
                        ((lh||lhu||sh)&&(ALU_Result[1]==0)&&(ALU_Result[0]==0))||
                        ((lw||sw)&&(ALU_Result[1]==0)&&(ALU_Result[0]==0)));    
                                       
    assign MemEnable2=(((lb||lbu||sb)&&(ALU_Result[1]==1)&&(ALU_Result[0]==0))||
                        ((lh||lhu||sh)&&(ALU_Result[1]==1)&&(ALU_Result[0]==0))||
                        ((lw||sw)&&(ALU_Result[1]==0)&&(ALU_Result[0]==0)));
                        
    assign MemEnable3=(((lb||lbu||sb)&&(ALU_Result[1]==1)&&(ALU_Result[0]==1))||
                        ((lh||lhu||sh)&&(ALU_Result[1]==1)&&(ALU_Result[0]==0))||
                        ((lw||sw)&&(ALU_Result[1]==0)&&(ALU_Result[0]==0)));  
